...
# Default Apache log path (common locations)
LOG_PATH="/var/log/apache2/access.log"

# Function to get timestamp from X minutes ago
get_minutes_ago() {
    local minutes=$1
    date -d "$minutes minutes ago" +[%d/%b/%Y:%H:%M
}
...
monitor_time_window() {
    ...
    ' "$LOG_PATH" | sort | uniq -c | sort -nr | head -n 10
...
monitor_attacks() {
    ...
    awk '$9 >= 400 && $9 < 500 {print $1}' "$LOG_PATH" | sort | uniq -c | sort -nr | head -n 10

    echo -e "\nTop IPs with 403/401 errors (potential brute force):"
    awk '$9 == 403 || $9 == 401 {print $1}' "$LOG_PATH" | sort | uniq -c | sort -nr | head -n 10

    echo -e "\nSuspicious patterns (SQL injection, admin pages, etc.):"
    grep -i "union\|select\|insert\|delete\|update\|admin\|wp-login\|\.php" "$LOG_PATH" |
...
detect_log_path() {
    ...
    echo "Error: Could not find Apache access log file"
    exit 1
}
...
main() {
    ...
    if [ ! -r "$LOG_PATH" ]; then
        echo "Error: Cannot read log file at $LOG_PATH"
        exit 1
    fi
